...
export function UserpageButtons(user: string) {
  ...
  if (user === 'Client') {
    ...
    buttons.push(['View Tickets', tickets, 'There are 3 new unclaimed tickets!'])
  }
    ...
    <>
      <div className="userpageButtons bg-white dark:bg-slate-800">
        <button className="flex" onClick={buttons[0][1]}>
          <div className='w-1/3 text-4xl py-40 text-cyan-800 dark:text-cyan-400'>{buttons[0][0]}</div>
          ...
          <div className='w-1/3 text-2xl py-40 text-cyan-800 dark:text-cyan-400'>{buttons[0][2]}</div>
        </button>
        <button className="flex" onClick={buttons[1][1]}>
          <div className='w-1/3 text-4xl py-40 text-cyan-800 dark:text-cyan-400'>{buttons[1][0]}</div>
          <img className='w-1/3 object-scale-down'src='https://ictzaak.nl/wp-content/uploads/2019/06/Helpdesk-ICTzaak-Blogpage.png'></img> 
          <div className='w-1/3 text-2xl py-40 text-cyan-800 dark:text-cyan-400'>{buttons[1][2]}</div>
        </button>
      </div>
    </>
  );
